...
import {
    ETH, BASE, getPoolsContract, POOLS_ADDR, getPoolData,
...
const Stake = (props) => {
    ...
    const [tokenList, setTokenList] = useState([BASE])
    ...
    const getData = async () => {
        ...
        const inputTokenData = await getTokenData(BASE, context.walletData)
        setStake1Data(await getStakeInputData(inputTokenData.balance, BASE))
                    ...
                    <Row style={paneStyles}>
                        <Col xs={24} style={colStyles}>
                            ...
                            <Row>
                                <Col xs={8}>
                                    {!approval1 &&
                                        <Center><Button type={'secondary'} onClick={unlockBase}>UNLOCK {stake1Data.symbol}</Button></Center>
                                    }
                                </Col>
                                <Col xs={8}>
                                {(approval1 && approval2) &&
