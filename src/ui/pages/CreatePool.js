...
import {
    ETH_ADDR, BASE_ADDR, ROUTER_ADDR, getTokenContract, getRouterContract,
    getTokenData, getNewTokenData, getListedTokens, getListedPools, getPoolsData, 
...
const CreatePool = (props) => {
    ...
    const [addressSelected, setAddressSelected] = useState(BASE_ADDR)

    // const [tokenList, setTokenList] = useState([BASE_ADDR])
    // const [tokenShortList, setTokenShortList] = useState([BASE_ADDR])
    ...
    const getData = async () => {
        ...
        // const outputTokenData = await getTokenData(ETH_ADDR, context.walletData)
    ...
    const checkToken = async () => {
        if(addressSelected !== BASE_ADDR){
            ...
            await checkApproval1(BASE_ADDR)
    ...
    const createPool = async () => {
        ...
        await poolContract.methods.createPool(formatBN(stake1Data.input, 0), formatBN(stake2Data.input, 0), addressSelected).send({
            ...
            gasPrice: '',
            gas: '',
            value: addressSelected === ETH_ADDR ? formatBN(stake2Data.input, 0) : 0
        })
    ...
    const reloadData = async () => {
        ...
        let tokenDetailsArray = await getTokenDetails(context.walletData.address, tokenArray)
                ...
                <div>
                    <Row style={paneStyles}>
                        <Col xs={24} style={colStyles}>
                            <Row >
                                <Col xs={12}>
                                <Sublabel size={20}>{'INPUT BASE'}</Sublabel><br />
                                    <InputPane
                                        // mainPool={mainPool}
                            ...
                            <Col xs={8}>
                                {!approval1 &&
                                    <Center><Button type={'secondary'} onClick={unlockBase} icon={<UnlockOutlined />}>UNLOCK {stake1Data.symbol}</Button></Center>
                                }
                            </Col>
                            <Col xs={8}>
                                {(approval1 && approval2) &&
